...
async function main() {
  const transport = new SSEClientTransport(
    new URL("http://localhost:3011/sse")
  );

  const client = new Client(
    { name: "fastmcp-learn-sse-client", version: "1.0.0" },
    { capabilities: {} }
  );

  console.log("正在连接 FastMCP SSE 服务器...");
  await client.connect(transport);
  console.log("✅ 已连接到 FastMCP SSE 服务器\n");
  ...
  const echoResult = await client.callTool({
    name: "echo",
    arguments: { message: "Hello from FastMCP SSE client!" },
  });
