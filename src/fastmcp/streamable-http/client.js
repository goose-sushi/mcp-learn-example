...
async function main() {
  ...
  const client = new Client(
    { name: "fastmcp-learn-streamable-http-client", version: "1.0.0" },
    { capabilities: {} }
  );

  console.log("正在连接 FastMCP Streamable HTTP 服务器...");
  await client.connect(transport);
  console.log("✅ 已连接到 FastMCP Streamable HTTP 服务器\n");
  ...
  const echoResult = await client.callTool({
    name: "echo",
    arguments: { message: "Hello from FastMCP Streamable HTTP client!" },
  });
